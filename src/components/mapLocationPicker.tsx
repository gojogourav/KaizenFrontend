...
interface PickedLocation {
  lat: number;
  lng: number;
  label: string; // "City, State" or full address
}
...
export const MapLocationPicker: React.FC<MapLocationPickerProps> = ({
  ...
  useEffect(() => {
    ...
    // Custom marker icon to avoid the broken default icon issue
  ...
  // Search handler (Nominatim forward geocode)
  ...
  const flyToResult = (result: any) => {
    ...
    const label = addr.slice(0, 3).join(",").trim();
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
      <div className="relative w-full max-w-2xl bg-[#0F1014] border border-white/15 rounded-3xl overflow-hidden shadow-2xl flex flex-col"
        ...
        <div className="px-4 py-3 border-b border-white/10 shrink-0 space-y-2">
          <div className="flex gap-2">
            ...
            <button onClick={handleSearch}
              disabled={searching}
              className="px-4 py-2 bg-white/10 hover:bg-white/15 border border-white/10 text-slate-200 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 font-mono">
              {searching ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : "Search"}
            </button>
            <button onClick={handleGps}
              disabled={gpsLoading}
              title="Use my GPS location"
              className="px-3 py-2 bg-[#E04F33]/20 hover:bg-[#E04F33]/30 border border-[#E04F33]/40 text-[#FF8A73] rounded-xl transition-all flex items-center gap-1.5">
              {gpsLoading
          ...
          {searchResults.length > 0 && (
            <div className="bg-[#0F1014] border border-white/15 rounded-xl overflow-hidden shadow-xl">
              {searchResults.map((r, i) => (
                <button key={i} onClick={() => flyToResult(r)}
                  className="w-full text-left px-3.5 py-2.5 text-xs text-slate-200 hover:bg-white/10 transition-colors border-b border-white/5 last:border-0 font-mono truncate">
                  {r.display_name}
                </button>
              ))}
            </div>
        ...
        <div className="flex-1 relative">
          ...
          {leafletLoaded && !picked && (
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 pointer-events-none">
              <div className="px-3 py-2 bg-black/70 rounded-full text-[10px] text-slate-300 font-mono border border-white/10 whitespace-nowrap">
        ...
        <div className="px-4 py-3 border-t border-white/10 shrink-0 flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2 shrink-0">
            <button onClick={onClose}
              className="px-4 py-2 bg-white/5 hover:bg-white/10 text-slate-300 rounded-xl text-xs font-bold border border-white/10 transition-all">
              Cancel
            </button>
            <button
