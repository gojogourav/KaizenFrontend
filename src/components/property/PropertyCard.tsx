...
export const PropertyCard: React.FC<PropertyCardProps> = ({
  ...
  const getStatusClasses = () => {
    if (statusDisplay === "AVAILABLE" || statusDisplay === "ACTIVE")
      return "bg-[#34D399]/10 text-[#34D399] border-[#34D399]/20";
    if (statusDisplay === "OCCUPIED" || statusDisplay === "LOCKED")
      return "bg-amber-500/10 text-amber-400 border-amber-500/20";
    if (statusDisplay === "UNDER CONTRACT")
  ...
  return (
    <motion.article
      ...
      className="group relative bg-[#14151A] rounded-[1.5rem] border border-white/5 hover:border-white/10 shadow-xl shadow-black/50 overflow-hidden flex flex-col justify-between cursor-pointer apple-specular h-full w-full min-w-0"
    >
      ...
      <div className="relative h-64 sm:h-[19rem] w-full overflow-hidden bg-[#0B0C10] shrink-0">
        <img
          src={coverImage}
          alt={deal.title}
          loading="lazy"
        ...
        <div className="absolute top-3.5 left-3.5 right-3.5 flex items-center justify-between z-10 gap-2 min-w-0">
          <div className="px-3.5 py-1.5 bg-[#0B0C10]/80 backdrop-blur-md rounded-lg text-[10px] font-bold tracking-widest text-slate-100 border border-white/10 flex items-center gap-1.5 shadow-lg font-mono uppercase min-w-0">
        ...
        <div className="absolute bottom-3.5 left-3.5 right-3.5 z-10 flex items-center justify-between min-w-0 gap-2">
          <div className="px-3.5 py-1.5 bg-[#0B0C10]/80 backdrop-blur-md border border-white/10 rounded-lg text-[10px] font-bold text-slate-300 tracking-wider font-mono flex items-center gap-2.5 shadow-lg min-w-0 truncate">
            ...
            {deal.squareFeet && (
              <>
                ...
                <span className="flex items-center gap-1.5 text-white truncate">
                  <Maximize2 className="w-3.5 h-3.5 text-[#E04F33] shrink-0" />
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-[#0B0C10]/80 backdrop-blur-md border border-emerald-500/20 rounded-lg text-[10px] font-bold text-emerald-400 font-mono shadow-lg shrink-0">
        ...
        <div className="min-w-0">
          <div className="flex justify-between items-start gap-4 mb-3 min-w-0">
            <div className="space-y-1.5 min-w-0 flex-1">
              <h3 className="font-serif font-bold text-lg leading-tight text-white group-hover:text-[#FF8A73] transition-colors duration-200 truncate">
                {deal.title}
              </h3>

              <div className="flex flex-wrap items-center gap-2 text-[11px] font-mono text-slate-400 uppercase tracking-wider">
                <div className="flex items-center gap-1 font-bold text-amber-400 shrink-0">
                  <Star className="w-3 h-3 fill-amber-400 stroke-amber-400" />
                  <span>{rating.toFixed(2)}</span>
                </div>
                <span className="text-white/20 hidden sm:inline shrink-0">
                  •
                </span>
                <span className="text-slate-400 hover:text-white transition-colors shrink-0">
                  {reviewCount} reviews
                </span>
              </div>
            </div>
          ...
          <p className="text-xs text-slate-400 leading-relaxed font-sans line-clamp-2 mt-2">
            {deal.description ||
              "Stunning turnkey property highly optimized for luxury stays and immediate revenue generation."}
          </p>
        </div>

        <div className="pt-4 border-t border-white/5 flex items-center justify-between gap-2 mt-auto min-w-0">
          <div className="flex items-center gap-1.5 text-[11px] text-slate-400 font-mono truncate">
            {activeListings.length > 0 ? (
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#0B0C10] border border-white/5 text-slate-300 text-[10px] font-bold uppercase tracking-widest shadow-inner shrink-0">
                <ShieldCheck className="w-3.5 h-3.5 text-[#E04F33]" />
                {activeListings.length} Channels
              </span>
            ) : (
              <span className="text-[10px] font-mono tracking-widest uppercase text-slate-500 truncate">
                {availabilityRange}
              </span>
            )}
          </div>

          <div className="inline-flex items-center gap-2 text-xs font-bold font-mono tracking-widest uppercase text-[#E04F33] group-hover:text-[#FF8A73] transition-colors shrink-0">
            <span>Explore</span>
