    ...
    <div className="space-y-6 text-slate-100 font-sans">
                    ...
                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] font-mono text-slate-400">/{blog.slug}</p>
                        </div>
                      </div>
                    </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md font-sans">
          <div className="relative w-full max-w-2xl bg-[#0F1014] border border-white/15 rounded-3xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto text-slate-100 space-y-5 apple-specular">
            ...
            <form onSubmit={handleSave} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block font-bold text-[#FF8A73] uppercase mb-1 font-mono">Title</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block font-bold text-[#FF8A73] uppercase mb-1 font-mono">Slug</label>
                  <input
                    type="text"
                    required
                    value={formData.slug}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block font-bold text-[#FF8A73] uppercase mb-1 font-mono">Author Name</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block font-bold text-[#FF8A73] uppercase mb-1 font-mono">Status</label>
                  <select
                    value={formData.status}
                    onChange={(e) => setFormData({ ...formData, status: e.target.value as any })}
                    className="w-full px-3.5 py-2.5 bg-[#0F1014] border border-white/10 rounded-xl text-slate-100 focus:outline-none focus:border-[#E04F33]"
                  >
              ...
              <div>
                <label className="block font-bold text-[#FF8A73] uppercase mb-1 font-mono">Cover Image URL</label>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3.5 py-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-100 focus:outline-none focus:border-[#E04F33]"
                  />
                  {formData.coverImageUrl && (
                    <img src={formData.coverImageUrl} alt="Preview" className="w-10 h-10 rounded-xl object-cover border border-white/20" />
                  )}
                </div>
              </div>

              <div>
                <label className="block font-bold text-[#FF8A73] uppercase mb-1 font-mono">
                ...
                <textarea
                  ...
                  className="w-full p-3.5 bg-white/5 border border-white/10 rounded-xl text-slate-100 focus:outline-none focus:border-[#E04F33] font-mono text-xs leading-relaxed"
                />
              </div>
